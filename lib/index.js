...
const transformToWriteFile = function (chunk, enc, cb) {
    ...
    writeStream.write(new Buffer.from(content, 'base64'), cb);
};
...
eventsEmitter.addListener('request', requestHandlers((req, res) => {
    req.pipe(new Transform({transform: transformToWriteFile})).pipe(res);
